...
/**
 * Parses and normalizes an LLM story response into the persisted payload.
 * Unknown part kinds (including `lens` — lenses must never be created by the
 * model) are dropped; a response with no usable parts is rejected.
 */
