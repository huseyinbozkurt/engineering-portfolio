/**
 * Versioned AI-story prompt builders, mirroring the insights registry: every
 * story records the prompt version it was generated with, and existing
 * versions are never edited — new behaviour means a new version entry.
 *
 * The content context is structural (the admin passes its content index), so
 * this package stays free of database dependencies.
 */

export interface AiStoryContentContext {
  ...
  caseStudies: Array<{ id: string; slug: string; title: string; excerpt: string }>;
  principles: Array<{ id: string; slug: string; title: string; summary: string }>;
  skills: Array<{ id: string; slug: string; name: string; category: string | null; summary: string }>;
...
export interface StoryPromptVersion {
  version: string;
  build(sourceStory: string, content: AiStoryContentContext): BuiltStoryPrompt;
}

export const AI_STORY_PROMPT_V1 = "ai-story-v1";

const storyPromptV1: StoryPromptVersion = {
  version: AI_STORY_PROMPT_V1,
  build(sourceStory, content) {
...
export const storyPromptVersions: Record<string, StoryPromptVersion> = {
  [AI_STORY_PROMPT_V1]: storyPromptV1,
};

export const latestStoryPromptVersion = AI_STORY_PROMPT_V1;

export function getStoryPromptVersion(version: string): StoryPromptVersion {
  const entry = storyPromptVersions[version];
  if (!entry) {
...
function summarizeExistingContent(content: AiStoryContentContext) {
  return {
